...
object GlobalSnapshotConsensusFunctions {
  ...
  def make[F[_]: Async: KryoSerializer: MonadThrow](
    ...
    def consumeSignedMajorityArtifact(signedArtifact: Signed[GlobalSnapshotArtifact]): F[Unit] =
      globalSnapshotStorage.save(signedArtifact)
    ...
    def createProposalArtifact(
      ...
      for {
        ...
        lastGSInfo <- globalSnapshotStorage.getGlobalSnapshotInfo(lastGS.ordinal)
        (scSnapshots, returnedSCEvents) = processStateChannelEvents(lastGSInfo, scEvents)
    ...
    private def processStateChannelEvents(
      previousGlobalSnapshotInfo: GlobalSnapshotInfo,
      ...
      val result = events
        ...
        .distinct
        .mapFilter { address =>
          previousGlobalSnapshotInfo.lastStateChannelSnapshotHashes
            .get(address)
